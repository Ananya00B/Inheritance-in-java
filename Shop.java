...
public class Shop {
	...
	protected boolean is_available;	    //To check if the good asked for, is in the list or not
	 
	public Shop() {
		//Default Constructor
	}
	 
	protected Shop (String[] items, double[] prices, int[] stocks) throws NoProperFormatException {
	 ...
	 protected void addItem(String itm, int qty){
		 checkItem(itm);
		 if(is_available) {
	...
	//Method to buy an item
	 ...
	 public void getPrice (String itm) {
		 double c = 0.0;
	     checkItem (itm);
	     if(is_available) {
	    	 for (int i=0; i<item_list.length; i++) {
	 ...
	 public void checkItem(String itm) throws NoItemException{
		 is_available = false;
		 for (int i=0; i<item_list.length; i++) {
			 if(item_list[i].equals(itm)) {
					is_available = true;
